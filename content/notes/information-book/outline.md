...
- **Transport vs. selection.** Is compression at scale driven by lossy transport across hops, or by selection pressures favoring compressed-and-resonant content? Worked through in [[transport-vs-selection]]. Current provisional answer (refined): selection is primary, but *want* (desire to engage) is the prime mover that drives a feedback loop in which preconditions, capacity, and selection criteria all accumulate together. The medium shapes that want at the root, which connects directly to the next foundational question. If this holds up, Chapter 5 needs rework, the manipulation surface argument gets sharper, political economy becomes load-bearing, and AI moves from optional to central.
...
**Chapter 5b (new, possible): Selection As The Other Engine.** Needed if selection turns out to be co-equal with or primary over transport. Memetic fitness, status signaling, identity reinforcement, action affordance, novelty, confirmation. Engage with Cailin O'Connor and James Owen Weatherall's *The Misinformation Age* and Hugo Mercier's *Not Born Yesterday*. If selection turns out to be primary, this absorbs Chapter 5 and the current Chapter 5 becomes a sub-section about the transport dynamics that operate downstream.
...
1. ~~**Pick a side on transport vs. selection.**~~ Drafted as [[transport-vs-selection]], with a refinement noting that want/desire is the prime mover and the medium shapes want at the root. Still pressure-testable.
